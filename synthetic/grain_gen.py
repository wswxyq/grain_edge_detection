...
class Grain:
    ...
    def __init__(self, grain_id, seed, lattice="hex"):
        ...
        if lattice == "hex":
            ...
            self.lattice_disp = np.array([[1.0, 0], [0, 1.0], [-1.0, 0], [0, -1.0]]).T
            self.phi = np.pi / 2
        else:
...
class Crystal:
    ...
    def __init__(self, ngrains=5, seed_minimum_distance=0.2, lattice="hex", d=0.02):
    ...
    def get_neighbour_candidate_sites(self, atom):
        ...
        for site in neighbour_sites:
            ...
            neighbouring_atoms_generator = self.sim_cells.neighbouring_atoms_generator(
                site
            )
    ...
    def save_atom_positions(self, filename="crystal.out"):
        """Save the atom diameter and all atom locations to a dictionary."""

        for atom in self.atoms:
            if atom.grain.grain_id not in self.container:
                self.container[atom.grain.grain_id] = []
            self.container[atom.grain.grain_id].append(atom.coords)
    ...
    def plot_crystal(
        ...
        for j, atoms in grains.items():
            if circular_atoms:
                patches = [
                    plt.Circle(atom.coords, radius=self.d / 2, lw=0) for atom in atoms
                ]
            else:
                patches = [
                    plt.Polygon(self._get_patch_vertices(atom), lw=0) for atom in atoms
                ]
            c = PatchCollection(patches, facecolor=colours[j % ncolours], **kwargs)
            ax.add_collection(c)
...
# %%
r = 0.01
crystal = Crystal(ngrains=80, seed_minimum_distance=0.05, lattice="square", d=r)
...
# %%
# create a small noise image and augment image along 2 axes
height = 200
width = 200
xl = []
yl = []
...
testimg = np.random.random((height, width)) / 2 + 0.2
# resize image
resized = cv2.resize(testimg, (500, 500), interpolation=cv2.INTER_CUBIC)
resized = np.stack((resized * 255,) * 3, axis=-1).astype(np.uint8)
resized = gaussian_filter(resized, sigma=1.)

plt.imshow(testimg, cmap="gray")
# %%
height = 500
width = 500
for i in crystal.container.keys():
    blank_image = np.zeros((height, width), np.uint8)
    for j in range(len(crystal.container[i])):
        x = int(crystal.container[i][j][0] * width)
        y = int(crystal.container[i][j][1] * height)
        cv2.circle(blank_image, (x, y), int(r * height), 255, 0)
    contours, _ = cv2.findContours(
        blank_image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
    )
    xl += list(contours[0][:, 0, 0])
    yl += list(contours[0][:, 0, 1])

imax = resized.shape[1]
jmax = resized.shape[0]
